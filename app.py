...
def clean_sheet(n, used):
    ...
    while name.lower() in used:
        suffix = f"_{counter}"; name = base[:31-len(suffix)] + suffix; counter += 1
    used.add(name.lower())
    return name
...
def extract_tab(url, tab):
    if f"/{tab}" not in url: url = url.rstrip('/') + f"/{tab}"
    videos = {}
            ...
            if info and 'entries' in info:
                for v in info['entries']:
                    ...
                    try: 
                        ...
                        dt = None
                    videos[vid_id] = {"Video Title": v.get('title') or "video", "Duration (HH:MM:SS)": hms(dur), "Video Link": f"https://www.youtube.com/watch?v={vid_id}", "Publish Date": dt, "seconds": dur, "type": tab}
        except: pass
    return videos
def get_all_channel_content(url, progress_bar):
    tabs, all_content = ["videos", "shorts", "streams"], {}
    for i, tab in enumerate(tabs):
        source = extract_tab(url, tab)
...
def extract_playlist(pl):
    ...
    if info and 'entries' in info:
        for v in info['entries']:
            ...
            data.append({"Video Title": v.get('title') or "video", "Duration (HH:MM:SS)": hms(dur), "Video Link": f"https://www.youtube.com/watch?v={vid_id}", "Publish Date": dt})
...
def build_excel(results, summary, total_channel, metadata):
    ...
    with pd.ExcelWriter(output, engine="openpyxl", date_format="YYYY-MM-DD", datetime_format="YYYY-MM-DD") as writer:
        ...
        df_sum = pd.DataFrame({"Category": [s[1] for s in summary], "Video Count": [s[4] for s in summary], "Duration": [hms(s[3]) for s in summary], "Link": [s[2] for s in summary]})
        ...
        for i, r in enumerate(summary, 2):
            ...
            if r[2].startswith("http"): ws_sum[f"D{i}"].hyperlink, ws_sum[f"D{i}"].style = r[2], "Hyperlink"
...
if st.button("Generate Archive", type="primary"):
    if not url: st.error("Please enter a URL")
    else:
        ...
        with st.spinner("Processing full channel archive..."):
            try:
                y_playlists = []
                with yt_dlp.YoutubeDL(get_ydl_opts(True)) as d:
                    info = d.extract_info(url.rstrip('/') + '/playlists', download=False)
                    if info and 'entries' in info:
                        for e in info['entries']:
                            if e: y_playlists.append({'name': e.get('title') or "playlist", 'url': e.get('url') or e.get('webpage_url')})
                all_content = get_all_channel_content(url, progress)
                results, summary, playlist_video_ids, used = [], [], set(), {"summary", "channel_info"}
                with ThreadPoolExecutor(max_workers=5) as ex:
                    futures = [ex.submit(extract_playlist, p) for p in y_playlists]
                    for i, f in enumerate(as_completed(futures)):
                        n, pl, d, t, ids = f.result()
                        playlist_video_ids.update(ids); sn = clean_sheet(n, used)
                        results.append((sn, n, pl, d, t)); summary.append((sn, n, pl, t, len(d)))
                for label, tab_type in {"Videos": "videos", "Shorts": "shorts", "Streams": "streams"}.items():
                    ...
                    if items:
                        sn = clean_sheet(label, used); dur = sum(i['seconds'] for i in items)
                        results.append((sn, label, url, items, dur)); summary.append((sn, label, url, dur, len(items)))
                total_duration = sum(s[3] for s in summary)
                metadata = {"Export Tool": "YTCore Archive", "Export Date": datetime.now().strftime("%Y-%m-%d %H:%M"), "Total Duration": hms(total_duration)}
                excel_file = build_excel(results, summary, total_duration, metadata)
                progress.progress(1.0); st.success("Export Ready!")
                st.download_button("Download Export", excel_file, f"YTCore_Archive_{datetime.now().strftime('%Y%m%d')}.xlsx")
